...
import { getAgentsList, getAgentsMap, getAutonomyUpgradeConfig, getBossInboxConfig, getConfig, resolveAgentId } from './config.js'
import { listAttentionStates } from './db.js'
...
const APPLYABLE_MARKDOWN_TARGETS = new Set(['SYSTEM_PROMPT.md', 'HEARTBEAT.md', 'AI_KNOWLEDGE_BASE.md'])
...
const STATUS_SYNC_NOISE_RE = /Edit:\s*`?in ~\/\.openclaw\/agent\/status\.json/i
const AUTONOMY_STEP_LABELS = {
  1: 'Level 1 / Approved-Only Auto Apply',
  2: 'Level 2 / Guarded Auto Approve',
  3: 'Level 3 / Progressive Canary',
  4: 'Level 4 / Full Autonomous',
}
const AUTONOMY_LEVEL_PROFILES = {
  1: {
    autoApproveEnabled: false,
    autoApproveAllowAdvisory: false,
    autoApplyAllowAdvisory: false,
    autoApplyAdvisoryMinConfidence: 0.92,
    autoApplyAdvisoryMinImpact: 90,
    autoApplyMinSuccessRate7d: 0,
    autoApplyMaxOpenCritical: 99,
    autoApplyMaxPerCycle: 8,
  ...
  2: {
    ...
    autoApproveAllowAdvisory: false,
    autoApplyAllowAdvisory: false,
    autoApplyAdvisoryMinConfidence: 0.9,
    autoApplyAdvisoryMinImpact: 88,
    autoApplyMinSuccessRate7d: 0.45,
    autoApplyMaxOpenCritical: 2,
    autoApplyMaxPerCycle: 2,
    canaryWindowHours: 24,
  },
  3: {
    autoApproveEnabled: true,
    autoApproveAllowAdvisory: true,
    autoApplyAllowAdvisory: true,
    autoApplyAdvisoryMinConfidence: 0.88,
    autoApplyAdvisoryMinImpact: 86,
    autoApplyMinSuccessRate7d: 0.6,
    autoApplyMaxOpenCritical: 1,
    autoApplyMaxPerCycle: 3,
    canaryWindowHours: 20,
  },
  4: {
    autoApproveEnabled: true,
    autoApproveAllowAdvisory: true,
    autoApplyAllowAdvisory: true,
    autoApplyAdvisoryMinConfidence: 0.85,
    autoApplyAdvisoryMinImpact: 82,
    autoApplyMinSuccessRate7d: 0.65,
    autoApplyMaxOpenCritical: 1,
    autoApplyMaxPerCycle: 5,
    canaryWindowHours: 16,
  },
}
...
function normalizeOutcomeScore(value) {
  ...
  return clampNumber((number + 1) / 2, 0, 1)
}

function scoreFeedbackEntry(entry = {}) {
  const direct = normalizeOutcomeScore(entry?.didImproveScore)
  if (direct !== null) return direct
  if (entry?.rollbackNeeded) return 0
  if (entry?.didImprove === true) return 0.7
  if (entry?.didImprove === false) return 0.2
  return null
}
...
function logReadonlySkip(stage, error) {
  console.warn(`[evolution] ${stage} skipped in read-only runtime: ${error?.code || 'ERR'} ${error?.message || ''}`.trim())
}

function ensureEvolutionCandidateDirs({ create = true } = {}) {
...
function isRecoverableDeliveredStatusNoise(entry = {}) {
  const status = String(entry?.status || '').trim().toLowerCase()
  const error = String(entry?.error || '')
  const delivered = entry?.delivered === true || String(entry?.deliveryStatus || '').trim().toLowerCase() === 'delivered'
  return status === 'error' && delivered && STATUS_SYNC_NOISE_RE.test(error)
}
...
function buildEventFromRun(job, agent, entry, existingEvent = null) {
  const effectiveStatus = isRecoverableDeliveredStatusNoise(entry) ? 'ok' : (entry.status || 'ok')
  const parsed = parseReportSummary(entry.summary || entry.error || '', effectiveStatus)
...
export function syncEvolutionArtifacts({ lookbackMs = 7 * DAY_MS, now = Date.now() } = {}) {
  try {
    ...
    logReadonlySkip('syncEvolutionArtifacts', error)
    return {
      processedRuns: 0,
      touchedAgents: [],
      degraded: true,
...
function getAutonomyPolicy() {
  const config = getAutonomyUpgradeConfig()
  const level = clampNumber(Number(config?.level || 1), 1, 4)
  const profile = AUTONOMY_LEVEL_PROFILES[level] || AUTONOMY_LEVEL_PROFILES[1]
  const boolOr = (value, fallback) => (
    value === null || value === undefined ? fallback : Boolean(value)
  )
  const numOr = (value, fallback, { min = null, max = null, round = false } = {}) => {
    const parsed = toNumberOrNull(value)
    let next = parsed === null ? fallback : parsed
  ...
  const autoApproveEnabled = Boolean(
    level >= 2 &&
    !config?.killSwitch &&
    boolOr(config?.autoApproveEnabled, profile.autoApproveEnabled),
  )
  const policy = {
    level,
    label: AUTONOMY_STEP_LABELS[level] || AUTONOMY_STEP_LABELS[1],
    mode: config?.mode || 'semi_auto',
    killSwitch: Boolean(config?.killSwitch),
    autoApproveEnabled,
    autoApproveAllowAdvisory: boolOr(config?.autoApproveAllowAdvisory, profile.autoApproveAllowAdvisory),
    autoApplyAllowAdvisory: boolOr(config?.autoApplyAllowAdvisory, profile.autoApplyAllowAdvisory),
    autoApproveMinConfidence: numOr(config?.autoApproveMinConfidence, 0.9, { min: 0, max: 1 }),
    autoApproveMinImpact: numOr(config?.autoApproveMinImpact, 85, { min: 0, max: 100 }),
    autoApproveMinSuccessRate14d: numOr(config?.autoApproveMinSuccessRate14d, 0.65, { min: 0, max: 1 }),
    autoApproveMaxOpenCritical: numOr(config?.autoApproveMaxOpenCritical, 0, { min: 0, round: true }),
    autoApplyAdvisoryMinConfidence: numOr(config?.autoApplyAdvisoryMinConfidence, profile.autoApplyAdvisoryMinConfidence, { min: 0, max: 1 }),
    autoApplyAdvisoryMinImpact: numOr(config?.autoApplyAdvisoryMinImpact, profile.autoApplyAdvisoryMinImpact, { min: 0, max: 100 }),
    autoApplyMinSuccessRate7d: numOr(config?.autoApplyMinSuccessRate7d, profile.autoApplyMinSuccessRate7d, { min: 0, max: 1 }),
    autoApplyMaxOpenCritical: numOr(config?.autoApplyMaxOpenCritical, profile.autoApplyMaxOpenCritical, { min: 0, round: true }),
    autoApplyMaxPerCycle: numOr(config?.autoApplyMaxPerCycle, profile.autoApplyMaxPerCycle, { min: 1, round: true }),
    canaryWindowHours: numOr(config?.canaryWindowHours, profile.canaryWindowHours, { min: 1, round: true }),
  }
  return policy
}

function getCanaryWindowMs(policy = getAutonomyPolicy()) {
  return Math.max(1, Number(policy?.canaryWindowHours || 24)) * 60 * 60 * 1000
}

function countOpenCriticalAttention(now = Date.now()) {
  return listAttentionStates().filter((state) => {
    if (!state || state.status !== 'open') return false
    if (!['blocked', 'risk'].includes(String(state.attentionType || ''))) return false
    const snoozedUntil = Number(state.snoozedUntil || 0)
    const nextReviewAt = Number(state.nextReviewAt || 0)
    if (snoozedUntil > now || nextReviewAt > now) return false
    return true
  }).length
}

function computeCandidateActionSuccessRate14d(candidate, now = Date.now(), allCandidates = []) {
  const horizon = now - (14 * DAY_MS)
  const candidates = Array.isArray(allCandidates) && allCandidates.length > 0 ? allCandidates : listCandidatePatchFiles()
  ...
  for (const entry of related) {
    for (const feedback of (entry.feedbackHistory || [])) {
      const at = Number(feedback?.at || 0)
      if (!at || at < horizon) continue
      const score = scoreFeedbackEntry(feedback)
      if (score === null) continue
      scores.push(score)
    }
    const fallback = scoreFeedbackEntry(entry)
    const fallbackAt = Number(entry.lastFeedbackAt || entry.updatedAt || 0)
    if (fallback !== null && fallbackAt >= horizon) {
...
function decorateCandidatePatch(candidate) {
  ...
  const candidateKind = candidate.candidateKind || 'recurring'
  const confidence = Number(candidate?.confidence || 0)
  const impact = Number(candidate?.estimatedImpact || 0)
  const allowAutoApplyByPolicy = !policy.killSwitch && policy.level >= 1
  const advisoryForApproveBlocked = candidateKind === 'advisory' && !policy.autoApproveAllowAdvisory
  const advisoryForApplyBlocked = candidateKind === 'advisory' && !policy.autoApplyAllowAdvisory
  const autoApproveReady = Boolean(
    !policy.killSwitch &&
    reviewStatus === 'pending' &&
    canaryStatus === 'none' &&
    !advisoryForApproveBlocked &&
    evolutionStatus !== 'blocked_by_missing_guard' &&
    allChecksPassed &&
    confidence >= policy.autoApproveMinConfidence &&
    impact >= policy.autoApproveMinImpact,
  )
  const autoApplyEligible = Boolean(
    allowAutoApplyByPolicy &&
    reviewStatus === 'approved' &&
    !applyStatus &&
    canaryStatus === 'none' &&
    !advisoryForApplyBlocked &&
    evolutionStatus !== 'blocked_by_missing_guard' &&
    (candidateKind !== 'advisory' || (
      confidence >= policy.autoApplyAdvisoryMinConfidence &&
      impact >= policy.autoApplyAdvisoryMinImpact
    )) &&
  ...
  const applyStatusLabel = applyStatus === 'applied'
    ? (canaryStatus === 'running' ? 'Applied / Canary Running' : 'Applied')
...
function listCandidatePatchFiles() {
  const { pendingDir } = ensureEvolutionCandidateDirs({ create: false })
...
export function reviewCandidatePatch({
  ...
  const reviewDryRun = buildCandidateDryRunReport(existing)
  const reviewChecks = buildCandidatePrereqChecks(existing, reviewDryRun)
  const reviewStatusGate = classifyCandidateEvolutionStatus(existing, reviewChecks, reviewDryRun)

  const next = {
    ...existing,
    dryRunReport: reviewDryRun,
    prereqChecks: reviewChecks,
    applyPrereqs: reviewChecks.filter((check) => !check.passed).map((check) => check.label),
    dryRunSummary: buildDryRunSummary(existing, reviewChecks, reviewStatusGate),
    evolutionStatus: reviewStatusGate,
    reviewStatus,
...
export function applyCandidatePatch({
  ...
  const policy = getAutonomyPolicy()
  const canaryWindowMs = getCanaryWindowMs(policy)
  ...
  const next = {
    ...
    canaryDeadlineAt: normalizedMode === 'auto' ? (now + canaryWindowMs) : (existing.canaryDeadlineAt || null),
...
export function unapplyCandidatePatch({
  ...
  const next = {
    ...existing,
    applyStatus: 'rolled_back',
    canaryStatus: existing.canaryStatus === 'running' ? 'rolled_back' : (existing.canaryStatus || 'none'),
    canaryUpdatedAt: now,
    rollbackReason: existing.rollbackReason || 'manual-unapply',
    rollbackNeeded: true,
    unappliedAt: now,
    unappliedBy: applier,
    appliedAt: null,
...
export function recordCandidatePatchOutcome({
  ...
  for (const file of files) {
    ...
    const feedback = {
      ...
      didImproveScore: toNumberOrNull(didImproveScore),
      businessDelta: toNumberOrNull(businessDelta),
      processScore: toNumberOrNull(processScore),
      businessScore: toNumberOrNull(businessScore),
    ...
    const next = {
      ...
      didImproveScore: toNumberOrNull(didImproveScore) ?? toNumberOrNull(existing.didImproveScore) ?? null,
      businessDelta: toNumberOrNull(businessDelta) ?? toNumberOrNull(existing.businessDelta) ?? null,
      processScore: toNumberOrNull(processScore) ?? toNumberOrNull(existing.processScore) ?? null,
      businessScore: toNumberOrNull(businessScore) ?? toNumberOrNull(existing.businessScore) ?? null,
    ...
    const negativeImprove = Number(next.didImproveScore) < 0
    if ((next.rollbackNeeded || negativeImprove) && next.applyStatus === 'applied') {
      const rolled = applyRollbackToCandidateRecord(next, {
        now,
        actor: reviewer,
        reason: next.rollbackNeeded ? 'rollback-needed' : 'did-improve-negative',
      })
      rolled.canaryStatus = next.canaryStatus === 'running' ? 'rolled_back' : (rolled.canaryStatus || next.canaryStatus || 'none')
      rolled.canaryUpdatedAt = now
      safeWriteJson(filePath, rolled)
      if (rolled.reviewArtifactPath) writeCandidateReviewArtifact(rolled)
      updated.push(decorateCandidatePatch(rolled))
      continue
    }
...
export function refreshCandidatePatches({ now = Date.now() } = {}) {
  ...
  try {
    ...
    if (!isReadonlyFsError(error)) throw error
    logReadonlySkip('refreshCandidatePatches', error)
    return listCandidatePatchFiles()
  }
}

function autoApprovePendingCandidates(candidates, now = Date.now()) {
  const policy = getAutonomyPolicy()
  if (!policy.autoApproveEnabled || policy.killSwitch) return []
  if (policy.level < 2) return []

  const openCritical = countOpenCriticalAttention(now)
  if (openCritical > policy.autoApproveMaxOpenCritical) return []

  const allCandidates = listCandidatePatchFiles()
  const approved = []
  for (const candidate of candidates) {
    if (!candidate) continue
    if ((candidate.reviewStatus || 'pending') !== 'pending') continue
    if (!candidate.autoApproveReady) continue
    if (candidate.candidateKind === 'advisory' && !policy.autoApproveAllowAdvisory) continue
    const successRate14d = computeCandidateActionSuccessRate14d(candidate, now, allCandidates)
    if (successRate14d < policy.autoApproveMinSuccessRate14d) continue

    try {
      const next = reviewCandidatePatch({
        id: candidate.id,
        reviewStatus: 'approved',
        reviewer: 'evolution-engine:auto-approve',
        reviewNote: `auto-approved(level=${policy.level}, success14d=${successRate14d.toFixed(2)}, openCritical=${openCritical})`,
        now,
      })
      if (next) approved.push(next)
    } catch {
      // keep as pending; UI can inspect review gate and evidence
    }
  }
  return approved
}

function computeAutoApplySuccessRate7d(candidates, now = Date.now()) {
  const autoApplied7d = candidates.filter((item) => {
    if (item.applyMode !== 'auto') return false
    const appliedAt = Number(item.autoAppliedAt || item.appliedAt || 0)
    return appliedAt > 0 && (now - appliedAt <= 7 * DAY_MS)
  })
  if (autoApplied7d.length === 0) return null
  const passed = autoApplied7d.filter((item) => item.canaryStatus === 'passed').length
  return Number((passed / autoApplied7d.length).toFixed(3))
}

function autoApplyApprovedCandidates(candidates, now = Date.now()) {
  const policy = getAutonomyPolicy()
  if (policy.killSwitch) return []
  if (policy.level < 1) return []
  const openCritical = countOpenCriticalAttention(now)
  if (openCritical > policy.autoApplyMaxOpenCritical) return []
  const successRate7d = computeAutoApplySuccessRate7d(candidates, now)
  if (successRate7d !== null && successRate7d < policy.autoApplyMinSuccessRate7d) return []

  const ranked = [...candidates]
    .filter((candidate) => candidate?.autoApplyEligible)
    .filter((candidate) => (candidate.reviewStatus || 'pending') === 'approved')
    .filter((candidate) => (candidate.canaryStatus || 'none') === 'none')
    .filter((candidate) => {
      if (candidate.candidateKind !== 'advisory') return true
      if (!policy.autoApplyAllowAdvisory) return false
      const confidence = Number(candidate.confidence || 0)
      const impact = Number(candidate.estimatedImpact || 0)
      return confidence >= policy.autoApplyAdvisoryMinConfidence && impact >= policy.autoApplyAdvisoryMinImpact
    })
    .sort((a, b) => {
      if (Number(b.estimatedImpact || 0) !== Number(a.estimatedImpact || 0)) return Number(b.estimatedImpact || 0) - Number(a.estimatedImpact || 0)
      if (Number(b.recurrence || 0) !== Number(a.recurrence || 0)) return Number(b.recurrence || 0) - Number(a.recurrence || 0)
      if (Number(b.confidence || 0) !== Number(a.confidence || 0)) return Number(b.confidence || 0) - Number(a.confidence || 0)
      return Number(b.updatedAt || 0) - Number(a.updatedAt || 0)
    })
    .slice(0, Math.max(1, Number(policy.autoApplyMaxPerCycle || 1)))
  ...
  for (const candidate of ranked) {
    try {
      const next = applyCandidatePatch({
        id: candidate.id,
        applier: 'evolution-engine:auto-apply',
        mode: 'auto',
        now,
      })
      if (next) applied.push(next)
    } catch {
      // keep candidate in pending lifecycle; preflight or apply guard already surfaced in candidate metadata
    }
  }
  return applied
}

function evaluateRunningCanaries(candidates, {
  now = Date.now(),
  events = null,
} = {}) {
  const policy = getAutonomyPolicy()
  ...
  const updated = []
  for (const candidate of candidates) {
    if (!candidate || candidate.applyStatus !== 'applied') continue
    if ((candidate.canaryStatus || 'none') !== 'running') continue

    const filePath = join(getEvolutionPendingDir(), `${candidate.id}.json`)
    const existing = safeReadJson(filePath, null)
    if (!existing) continue

    const baseline = existing.canaryBaseline || {
      blockerPressure: 0,
      riskPressure: 0,
      qualityRegressionCount: 0,
    }
    const currentPressure = summarizeAgentCanaryPressure(existing.agentId, now, events)
    const blockedRiskIncreased = Number(currentPressure.riskPressure || 0) > Number(baseline.riskPressure || 0)
    const qualityRegressionIncreased = Number(currentPressure.qualityRegressionCount || 0) > Number(baseline.qualityRegressionCount || 0)
    const didImproveNegative = Number(existing.didImproveScore) < 0

    let next = {
      ...existing,
      lastCanaryCheckAt: now,
      canaryUpdatedAt: now,
      updatedAt: now,
    }

    if (blockedRiskIncreased || qualityRegressionIncreased || didImproveNegative) {
      const reason = didImproveNegative
        ? 'did-improve-negative'
        : blockedRiskIncreased
          ? 'blocked-risk-net-increase'
          : 'quality-regression-increase'
      next = applyRollbackToCandidateRecord(next, {
        now,
        actor: 'evolution-engine:canary',
        reason,
      })
    } else if (now >= Number(existing.canaryDeadlineAt || (Number(existing.canaryStartedAt || 0) + canaryWindowMs))) {
      next.canaryStatus = 'passed'
      next.canaryUpdatedAt = now
      next.rollbackReason = null
      next.rollbackNeeded = false
      next.updatedAt = now
    }
...
function syncCandidateLifecycle(candidates, now = Date.now()) {
  const stage1 = listCandidatePatchFiles()
  autoApprovePendingCandidates(stage1, now)
  const stage2 = listCandidatePatchFiles()
  autoApplyApprovedCandidates(stage2, now)
  const allEvents = loadAllEvolutionEvents()
  evaluateRunningCanaries(listCandidatePatchFiles(), { now, events: allEvents })
  return listCandidatePatchFiles()
}

function buildAutonomyMetrics(candidates = [], now = Date.now()) {
  const autoApproved24h = candidates.filter((item) => {
    const reviewedAt = Number(item.reviewedAt || 0)
    return item.reviewedBy === 'evolution-engine:auto-approve' && reviewedAt > 0 && (now - reviewedAt <= DAY_MS)
  })
  const autoApplySuccessRate7d = computeAutoApplySuccessRate7d(candidates, now) ?? 0
  return {
    autoApproved24h: autoApproved24h.length,
    canaryOpenCount: candidates.filter((item) => item.canaryStatus === 'running').length,
    autoApplySuccessRate7d: Number(autoApplySuccessRate7d.toFixed(3)),
    openCriticalAttentionCount: countOpenCriticalAttention(now),
  }
}
...
export function buildEvolutionSnapshot({ now = Date.now() } = {}) {
  ...
  const candidatePatches = syncCandidateLifecycle(refreshCandidatePatches({ now }), now)
  const statuses = []
  const growthSignals = []
  ...
  const autonomyMetrics = buildAutonomyMetrics(candidatePatches, now)
...
export function buildEvolutionAttentionItems({ now = Date.now(), snapshot = null } = {}) {
  ...
  for (const status of currentSnapshot.agentEvolutionStatus || []) {
    ...
    const qualityRegressionMinCount = Number(getBossInboxConfig().qualityRegressionMinCount || 2)
    if (Number(status.qualityRegressionCount || 0) >= qualityRegressionMinCount) {
      items.push({
  ...
  for (const candidate of currentSnapshot.candidatePatches || []) {
    if (!candidate?.agentId) continue
    if (candidate.applyStatus !== 'rolled_back') continue
    if (!candidate.rollbackNeeded) continue
    const rollbackAt = Number(candidate.unappliedAt || candidate.canaryUpdatedAt || candidate.updatedAt || 0)
    if (rollbackAt <= 0 || now - rollbackAt > DAY_MS) continue
    const agent = agentMap[candidate.agentId] || {}
    const channel = agent.bindings?.find((binding) => String(binding).startsWith('discord')) || null
    items.push({
      id: `evolution:risk:${candidate.agentId}`,
      requestId: null,
      source: 'evolution',
      eventId: candidate.id,
      escalationReason: 'canary-rollback',
      signalCount: 1,
      signalScore: 92,
      categories: ['canary-rollback'],
      agentId: candidate.agentId,
      agentName: candidate.agentName || agent.name || candidate.agentId,
      agentEmoji: agent.emoji || '🤖',
      agentColor: agent.color || '#888',
      bindings: agent.bindings || [],
      channel,
      ...
      needsDecision: false,
      state: 'evolution',
      title: trimText(`${candidate.agentName || candidate.agentId} canary rollback`, 80),
      detail: trimText(candidate.rollbackReason || 'Auto-apply canary detected regression and rolled back automatically.', 200),
...
export function buildStaleAttentionItems({ now = Date.now() } = {}) {
  const bossInboxConfig = getBossInboxConfig()
  const suppressSources = Array.isArray(bossInboxConfig.suppressSources) ? bossInboxConfig.suppressSources : []
  if (suppressSources.includes('stale-agent')) {
    return []
  }
...
export function runNightlyEvolutionPromotion({ now = Date.now() } = {}) {
  ...
  try {
    const events = loadAllEvolutionEvents()
    ensureEvolutionCandidateDirs()
    const candidates = syncCandidateLifecycle(writeCandidatePatches(buildCandidateRecords(events, now)), now)
    const autonomyPolicy = getAutonomyPolicy()
    const autonomyMetrics = buildAutonomyMetrics(candidates, now)
    ...
    if (!isReadonlyFsError(error)) throw error
    logReadonlySkip('runNightlyEvolutionPromotion', error)
    return {
      sync,
      candidatePatches: [],
      autonomyPolicy: getAutonomyPolicy(),
      autonomyMetrics: buildAutonomyMetrics([], now),
      promotedPatterns: [],
      memoryEvent: null,
      knowledgeBasePath: getKnowledgeBasePath(),
      pendingDir: getEvolutionPendingDir(),
