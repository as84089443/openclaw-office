...
export function loadGatewayOperatorIdentity() {
  ...
  const openclawDir = process.env.OPENCLAW_DIR || defaultDir
  let devicePath = join(openclawDir, 'identity', 'device.json')
  let authPath = join(openclawDir, 'identity', 'device-auth.json')

  if (!existsSync(devicePath)) {
    if (openclawDir !== defaultDir) {
      devicePath = join(defaultDir, 'identity', 'device.json')
      authPath = join(defaultDir, 'identity', 'device-auth.json')
    }
