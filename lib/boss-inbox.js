...
function getDisplayAgentMeta(agentMap, agentId) {
  return agentMap[agentId] || VIRTUAL_AGENT_META[agentId] || null
}
...
const MAINTENANCE_STALE_THRESHOLD_MS = 36 * 60 * 60 * 1000
const LOBSTER_MEMORY_STALE_THRESHOLD_MS = 2 * 60 * 60 * 1000
const DIRECT_INGRESS_REQUEST_SOURCES = new Set([
  'discord_gateway_autonomous',
  'telegram_webhook',
])
const HUMAN_API_REQUEST_FROM_RE = /^(boss|brian|志宏|志宏 brian)$/i
const INTERNAL_REQUEST_FROM_RE = /(office qa|system-smoke|boss inbox|automation|maintenance)/i
const INTERNAL_REQUEST_CONTENT_RE = /(\[smoke test\]|dispatch smoke test|office dispatch pipeline check)/i
...
function classifyLobsterTrackOrigin(task = {}, request = null) {
  const requestSource = String(request?.source || '').trim().toLowerCase()
  const requestFrom = String(request?.from || '').trim()
  const requestContent = trimText(request?.content || task?.title || task?.detail || '', 160)

  if (DIRECT_INGRESS_REQUEST_SOURCES.has(requestSource)) {
    return {
      originGroup: 'direct_ingress',
      originLabel: requestSource === 'discord_gateway_autonomous' ? 'Discord 直接交辦' : 'Telegram 直接交辦',
      originDescription: requestFrom ? `${requestFrom} 直接送進 formal workflow` : '直接送進 formal workflow',
  ...
  if (requestSource === 'api') {
    if (INTERNAL_REQUEST_FROM_RE.test(requestFrom) || INTERNAL_REQUEST_CONTENT_RE.test(requestContent)) {
      return {
        originGroup: 'background',
        originLabel: '系統內部驗證',
        originDescription: requestFrom ? `${requestFrom} 觸發的內部 smoke / 驗證題` : '內部 smoke / 驗證題',
        requestSource: requestSource || null,
        requestFrom: requestFrom || null,
      }
    }
    if (HUMAN_API_REQUEST_FROM_RE.test(requestFrom)) {
      return {
        originGroup: 'manual_request',
        originLabel: '手動 / API 交辦',
        originDescription: requestFrom ? `${requestFrom} 透過 API 或 Office 建立` : '透過 API 或 Office 建立',
        requestSource: requestSource || null,
        requestFrom: requestFrom || null,
      }
    }
  ...
  if (requestSource === 'boss-inbox') {
    return {
      originGroup: 'background',
      originLabel: 'Boss Inbox 跟進題',
      originDescription: '由注意事項卡片自動轉出的後續任務',
      requestSource: requestSource || null,
      requestFrom: requestFrom || null,
    }
  }
  ...
  return {
    originGroup: 'background',
    originLabel: requestSource ? `${requestSource} 背景任務` : '背景自治任務',
    originDescription: requestFrom ? `${requestFrom} 觸發的背景流程` : '背景自治或內部流程',
    requestSource: requestSource || null,
    requestFrom: requestFrom || null,
  }
}
...
function parseJsonSnapshot(filePath) {
  if (!filePath || !existsSync(filePath)) return null
  try {
    return JSON.parse(readFileSync(filePath, 'utf8'))
  } catch (error) {
    console.error('[boss-inbox] Failed to parse JSON snapshot:', filePath, error.message)
    return null
  }
}

function resolveSnapshotReportPath(snapshot, filePath) {
  const reportedPath = String(snapshot?.reportPath || '').trim()
...
function getMaintenanceReportDirCandidates() {
  const officeDbPath = String(process.env.OPENCLAW_OFFICE_DB_PATH || '').trim()
  return uniqueList([
    process.env.OPENCLAW_MAINTENANCE_REPORT_DIR,
    officeDbPath ? join(dirname(officeDbPath), 'maintenance-reports') : null,
    join(process.cwd(), 'data', 'maintenance-reports'),
    getConfig().openclaw?.home ? join(getConfig().openclaw.home, 'artifacts', 'maintenance', 'reports') : null,
  ])
}

function resolveMaintenanceReportSnapshot(fileName) {
  const candidates = getMaintenanceReportDirCandidates()
  for (const reportsDir of candidates) {
    const filePath = join(reportsDir, fileName)
    const snapshot = parseJsonSnapshot(filePath)
  ...
  const reportsDir = candidates[0] || join(process.cwd(), 'data', 'maintenance-reports')
  return {
    reportsDir,
...
function getMaintenanceReportsDir() {
  return getMaintenanceReportDirCandidates().find((reportsDir) => existsSync(reportsDir))
    || getMaintenanceReportDirCandidates()[0]
    || join(process.cwd(), 'data', 'maintenance-reports')
}
...
function getRootMaintenanceSnapshot() {
  const reportsDir = getMaintenanceReportsDir()
  const hygieneRef = resolveMaintenanceReportSnapshot('latest-root-hygiene.json')
  const cleanupRef = resolveMaintenanceReportSnapshot('latest-root-maintenance.json')
  const routeRef = resolveMaintenanceReportSnapshot('latest-copilot-route.json')
  const hygiene = hygieneRef.snapshot
  const cleanup = cleanupRef.snapshot
  const route = routeRef.snapshot
  const unexpectedRootFiles = Array.isArray(hygiene?.issues?.unexpectedRootFiles) ? hygiene.issues.unexpectedRootFiles : []
  const uncoveredTopLevelDirectories = Array.isArray(hygiene?.issues?.uncoveredTopLevelDirectories) ? hygiene.issues.uncoveredTopLevelDirectories : []
  const missingReadmes = Array.isArray(hygiene?.issues?.missingReadmes) ? hygiene.issues.missingReadmes : []
  const hygieneIssueCount = unexpectedRootFiles.length + uncoveredTopLevelDirectories.length + missingReadmes.length
  const cleanupCandidateCount = Number(cleanup?.summary?.candidateCount || 0)
  const cleanupCandidateBytes = Number(cleanup?.summary?.candidateBytes || 0)
  const rootGeneratedAt = Math.max(
    toTimestampOrNull(hygiene?.generatedAt) || 0,
    toTimestampOrNull(cleanup?.generatedAt) || 0,
    0,
  )
  const routeGeneratedAt = toTimestampOrNull(route?.generatedAt) || 0
  const generatedAt = Math.max(rootGeneratedAt, routeGeneratedAt, 0) || Date.now()
  const rootReportPresent = Boolean(hygiene || cleanup)
  const routeReportPresent = Boolean(route)
  const rootSnapshotAgeMs = rootGeneratedAt > 0 ? Math.max(0, Date.now() - rootGeneratedAt) : Infinity
  const routeSnapshotAgeMs = routeGeneratedAt > 0 ? Math.max(0, Date.now() - routeGeneratedAt) : Infinity
  const rootStale = !rootReportPresent || rootSnapshotAgeMs > MAINTENANCE_STALE_THRESHOLD_MS
  const routeStale = !routeReportPresent || routeSnapshotAgeMs > MAINTENANCE_STALE_THRESHOLD_MS
  const routeDegraded = !routeStale && (
    String(route?.status || '').toLowerCase() !== 'ok' ||
    String(route?.localHealth || '').toLowerCase() !== 'ok' ||
    String(route?.publicHealth || '').toLowerCase() !== 'ok' ||
    String(route?.publicBossInbox || '').toLowerCase() !== 'ok' ||
    Number(route?.strayCloudflaredCount || 0) > 0
  )
  const stale = rootStale || routeStale
  ...
  return {
    ...
    generatedAt,
    snapshotAgeMs: Math.max(rootSnapshotAgeMs, routeSnapshotAgeMs),
    rootSnapshotAgeMs,
    routeSnapshotAgeMs,
    rootStale,
    routeStale,
    ...
    status: stale
      ...
      ? 'issues'
      : routeDegraded
      ? 'copilot-route-degraded'
      : (cleanupCandidateCount > 0 ? 'cleanup-pending' : 'clean'),
    reportPaths: {
      hygiene: resolveSnapshotReportPath(hygiene, hygieneRef.filePath || join(reportsDir, 'latest-root-hygiene.json')),
      cleanup: resolveSnapshotReportPath(cleanup, cleanupRef.filePath || join(reportsDir, 'latest-root-maintenance.json')),
      route: resolveSnapshotReportPath(route, routeRef.filePath || join(reportsDir, 'latest-copilot-route.json')),
    },
    issueBreakdown: {
      unexpectedRootFiles,
    ...
    routeSummary: {
      hostname: route?.hostname || null,
      status: String(route?.status || (routeDegraded ? 'degraded' : (routeStale ? 'stale' : 'unknown'))).toLowerCase(),
      localHealth: String(route?.localHealth || 'unknown').toLowerCase(),
      publicHealth: String(route?.publicHealth || 'unknown').toLowerCase(),
      publicBossInbox: String(route?.publicBossInbox || 'unknown').toLowerCase(),
      dockerPublicStatus: String(route?.dockerPublicStatus || 'unknown').toLowerCase(),
      launchdStatus: String(route?.launchdStatus || 'unknown').toLowerCase(),
      strayCloudflaredCount: Number(route?.strayCloudflaredCount || 0),
    },
  }
}

function buildMaintenanceAttentionItems(agentMap) {
  const snapshot = getRootMaintenanceSnapshot()
  if (snapshot.status === 'clean') return { snapshot, items: [] }

  const agentId = resolveAgentId('admin')
  const agent = agentMap[agentId] || agentMap[getPrimaryAgentId()] || {}
  const hasHygieneIssues = snapshot.hygieneIssueCount > 0
  const hasRouteIssues = snapshot.routeDegraded || snapshot.routeStale
  const routeSignalCount = hasRouteIssues ? Math.max(1, snapshot.routeSummary.strayCloudflaredCount || 0) : 0
  const cleanupIsLarge = snapshot.cleanupCandidateCount >= 10 || snapshot.cleanupCandidateBytes >= 50 * 1024 * 1024
  const attentionType = snapshot.stale
    ? ATTENTION_TYPES.RISK
    : hasHygieneIssues
    ? ATTENTION_TYPES.RISK
    : hasRouteIssues
    ? ATTENTION_TYPES.RISK
    : (cleanupIsLarge ? ATTENTION_TYPES.BLOCKED : ATTENTION_TYPES.DIGEST_ONLY)
  const priority = ATTENTION_SEVERITY[attentionType] || ATTENTION_SEVERITY[ATTENTION_TYPES.DIGEST_ONLY]
  const detailParts = []

  if (snapshot.rootStale) {
    detailParts.push(`root maintenance reports stale ${Math.round(snapshot.rootSnapshotAgeMs / (60 * 60 * 1000))}h`)
  }
  if (snapshot.routeStale) {
    detailParts.push(`copilot route report stale ${Math.round(snapshot.routeSnapshotAgeMs / (60 * 60 * 1000))}h`)
  }
  if (hasHygieneIssues) {
    detailParts.push(
      `root hygiene issues ${snapshot.hygieneIssueCount} 項（unexpected files ${snapshot.issueBreakdown.unexpectedRootFiles.length}、uncovered dirs ${snapshot.issueBreakdown.uncoveredTopLevelDirectories.length}、missing READMEs ${snapshot.issueBreakdown.missingReadmes.length}）`,
    )
  }
  if (snapshot.routeDegraded) {
    detailParts.push(
      `copilot route degraded（local ${snapshot.routeSummary.localHealth} / public ${snapshot.routeSummary.publicHealth} / boss ${snapshot.routeSummary.publicBossInbox} / stray ${snapshot.routeSummary.strayCloudflaredCount}）`,
    )
  }
  if (snapshot.cleanupCandidateCount > 0) {
    detailParts.push(`cleanup pending ${snapshot.cleanupCandidateCount} files / ${humanSize(snapshot.cleanupCandidateBytes)}`)
  }
  detailParts.push(`reports: ${snapshot.reportPaths.hygiene} / ${snapshot.reportPaths.cleanup} / ${snapshot.reportPaths.route}`)
  const title = hasHygieneIssues
    ? 'Workspace hygiene 需要處理'
    : snapshot.routeDegraded
      ? 'Copilot route 需要處理'
      : snapshot.routeStale && !snapshot.rootStale
        ? 'Copilot route 報告過舊'
        : snapshot.stale
          ? 'Workspace maintenance 報告過舊'
          : `Workspace cleanup 待整理（${snapshot.cleanupCandidateCount} files）`

  return {
    snapshot,
    items: [
      {
        id: 'maintenance:root-workspace',
        source: 'maintenance',
        latestEventId: null,
        signalCount: Math.max(snapshot.hygieneIssueCount + snapshot.cleanupCandidateCount + routeSignalCount, 1),
        signalScore: priority,
        categories: uniqueList([
          hasHygieneIssues ? 'root-hygiene' : null,
          snapshot.cleanupCandidateCount > 0 ? 'cleanup-candidates' : null,
          hasRouteIssues ? 'copilot-route' : null,
        ]),
        agentId,
        ...
        attentionType,
        priority,
        severity: priority + Math.min(snapshot.hygieneIssueCount, 5) + Math.min(snapshot.cleanupCandidateCount, 8) + (hasRouteIssues ? 6 : 0),
        commercialValue: 0,
        needsDecision: false,
        state: 'open',
        title,
        detail: trimText(detailParts.join(' / '), 200),
        createdAt: snapshot.generatedAt,
        updatedAt: snapshot.generatedAt,
        unresolved: true,
        maintenanceStatus: snapshot.status,
        hygieneIssueCount: snapshot.hygieneIssueCount,
        cleanupCandidateCount: snapshot.cleanupCandidateCount,
        cleanupCandidateBytes: snapshot.cleanupCandidateBytes,
        copilotRouteStatus: snapshot.routeSummary.status,
        copilotRouteDegraded: snapshot.routeDegraded,
        copilotRouteStale: snapshot.routeStale,
        reportPaths: snapshot.reportPaths,
        escalationReason: snapshot.routeDegraded
          ? 'copilot-route'
          : snapshot.routeStale && !snapshot.rootStale
            ? 'copilot-route-stale'
            : snapshot.stale
              ? 'maintenance-stale'
              : (hasHygieneIssues ? 'root-hygiene' : 'cleanup-pending'),
      },
    ],
  }
}
...
function toAttentionStateRow(item, existing = null) {
  ...
  const nextSignalScore = attentionSignalScore(item)
  const previousSignalScore = Number(existing?.signalScoreMax || 0)
  const previousSignalCount = Number(existing?.signalCount || 0)
  const hasExistingLinkedRequest = Boolean(existing?.linkedRequestId)
  const hasExistingLinkedTask = Boolean(existing?.linkedTaskId)
  const signalFresh = (
    latestEventChanged
    || nextSignalScore > previousSignalScore
    || computedSignalCount > previousSignalCount
    || (hasExistingLinkedRequest && String(item.linkedRequestId || '') !== String(existing?.linkedRequestId || ''))
    || (hasExistingLinkedTask && String(item.linkedTaskId || '') !== String(existing?.linkedTaskId || ''))
    || String(item.attentionType || '') !== String(existing?.attentionType || '')
  )
  ...
  if (status === ATTENTION_STATUS.RESOLVED && signalFresh && latestSeenAt > Number(existing?.resolvedAt || 0)) {
  ...
  return {
    ...
    signalCount,
    signalScoreMax: Math.max(previousSignalScore, nextSignalScore),
    categories,
    snoozedUntil,
...
function mergeAttentionItemWithState(item, state, tasksById = new Map(), requestsById = new Map()) {
  ...
  const linkedTaskUpdatedAt = linkedTask
    ? Math.max(
        Number(linkedTask.lastUpdate || 0),
        Number(linkedTask.memoryUpdatedAt || 0),
        Number(linkedTask.continuationCheckedAt || 0),
        Number(linkedTask.completedAt || 0),
        Number(linkedTask.startedAt || 0),
        Number(linkedTask.createdAt || 0),
      ) || null
    : null
...
function inferTaskWorkflowPhase(task = {}, request = null) {
  const status = String(task?.status || request?.state || 'open').toLowerCase()
  const consensusStatus = String(task?.consensus?.status || '').toLowerCase()
  const reviewerResultCount = Number(task?.reviewerResults?.length || 0)
  const riskTier = String(task?.riskTier || '').toLowerCase()
  const activeReviewerChildren = getActiveReviewerChildren(task)
  const waitingReviewer = (
    activeReviewerChildren.length > 0
    || (
    task?.pendingAction === 'continue_after_reply'
    && (
      ['pending_review', 'needs_more_review'].includes(consensusStatus)
      || (reviewerResultCount === 0 && ['medium', 'high', 'irreversible'].includes(riskTier))
    )
    )
  )

  if (String(task?.resolutionSource || '').toLowerCase() === 'superseded') return 'superseded'
  if (status === 'completed') return 'completed'
  if (status === 'failed') return 'failed'
  if (waitingReviewer && (!task?.humanGateReason || isAutoGeneratedHumanGateReason(task.humanGateReason))) {
    return 'waiting_reviewer'
  }
  if (task?.humanGateReason) return 'waiting_human_gate'
  if (task?.continuationRequired && task?.pendingAction === 'continue_after_reply') return 'awaiting_continuation'
  if (task?.pendingAction === 'start_work' || ['pending', 'assigned', 'task_created'].includes(status)) return 'queued'
  if (status === 'in_progress') return 'running'
  return status || 'open'
}

function isResearchOperatorTrack(task = null) {
  if (!task || String(task.taskType || 'primary') !== 'primary') return false
  return resolveAgentId(task.assignedAgent) === 'research-fish'
    || String(task.operatorMode || task.brainState?.operatorMode || task.delegationPlan?.operatorMode || '').toLowerCase() === 'strict'
    || String(task.outputContract || task.brainState?.outputContract || task.delegationPlan?.outputContract || '') === 'research_operator_v1'
}

function inferResearchOperatorPhase(task = {}, request = null) {
  if (!isResearchOperatorTrack(task)) return null

  const workflowPhase = inferTaskWorkflowPhase(task, request)
  const activeChildren = getChildTasks(task.id, 100)
    .filter((child) => !['completed', 'failed'].includes(String(child.status || '').toLowerCase()))
  const hasVerifier = activeChildren.some((child) => ['verifier', 'sidecar_review'].includes(String(child.taskType || '')))
  const hasDelegatedFollowup = activeChildren.some((child) => String(child.taskType || '') === 'worker_subtask')
  const delegationStatus = String(task?.delegationPlan?.currentStatus || '').toLowerCase()

  if (workflowPhase === 'waiting_human_gate' || task?.humanGateReason) return 'waiting_human_gate'
  if (hasVerifier || delegationStatus === 'verifying') return 'verifying'
  if (hasDelegatedFollowup || delegationStatus === 'delegating') return 'delegating'
  if (['completed', 'failed', 'superseded'].includes(workflowPhase)) return null
  return 'running_research'
}
...
function syncAttentionItemsWithState(items, tasksById = new Map(), requestsById = new Map(), agentMap = getAgentsMap()) {
  ...
  for (const [id, state] of stateMap.entries()) {
    ...
    if (state.source === 'maintenance') {
      const closedAt = Date.now()
      const savedState = upsertAttentionState({
        ...state,
        status: ATTENTION_STATUS.RESOLVED,
        closedReason: state.closedReason || 'auto-resolved-clean',
        resolvedAt: closedAt,
        updatedAt: closedAt,
      })
      stateMap.set(id, savedState)
      continue
    }
...
function buildAttentionGovernanceSeed(item, draft = {}) {
  const attentionType = String(draft.attentionType || item?.attentionType || ATTENTION_TYPES.DIGEST_ONLY)
  const riskTier = attentionType === ATTENTION_TYPES.RISK
    ? 'high'
    : attentionType === ATTENTION_TYPES.DECISION || attentionType === ATTENTION_TYPES.OPPORTUNITY
      ? 'medium'
      : 'low'
  return {
    riskTier,
    retryBudget: riskTier === 'high' ? 1 : (riskTier === 'medium' ? 2 : 3),
    retryCount: 0,
    escalationLevel: riskTier === 'high' ? 1 : 0,
    autoContinueAllowed: riskTier !== 'high',
    autoApplyAllowed: riskTier === 'low',
    humanGateReason: item?.needsDecision ? '這題需要老闆拍板後再推進。' : null,
  }
}
...
function createWorkflowTaskFromAttention(item, overrides = {}) {
  ...
  const task = createTask({
    ...
    needsDecision: draft.needsDecision,
    estimatedValue: draft.estimatedValue,
    ...buildAttentionGovernanceSeed(item, draft),
  })
  ...
  const drafts = items.map((item) => {
    ...
    return {
      ...
      hint: {
        ...
        actionScores: scoredActions
          .map((entry) => ({
            action: entry.action,
            score: Number(entry.score.toFixed(3)),
            expectedSuccess: Number(Number(entry.expectedSuccess).toFixed(3)),
          }))
...
function buildAnomalies(attentionItems, dateKey = localDateKey()) {
  ...
  const maintenanceItem = attentionItems.find((item) => item.unresolved && item.source === 'maintenance')
...
function inferAttentionType({ request, task, text, agentId }) {
  ...
  const consensusStatus = String(task?.consensus?.status || '').toLowerCase()
  const blockers = Array.isArray(task?.brainState?.blockers) ? task.brainState.blockers.length : 0
  const workflowPhase = inferTaskWorkflowPhase(task, request)
  if (request?.needsDecision || task?.needsDecision || DECISION_RE.test(text)) {
    return ATTENTION_TYPES.DECISION
  }
  if (workflowPhase === 'waiting_reviewer') {
    return ATTENTION_TYPES.DIGEST_ONLY
  }
  if (task?.humanGateReason) {
    return consensusStatus === 'human_gate'
      ? ATTENTION_TYPES.DECISION
      : ATTENTION_TYPES.BLOCKED
  }
  if (['blocked', 'conflict'].includes(consensusStatus) || blockers > 0 || workflowPhase === 'failed') {
    return ATTENTION_TYPES.BLOCKED
  }
...
function buildRequestAttentionItem(request, task, eventsByRequest, agentMap) {
  ...
  const updatedAt = Math.max(
    Number(task?.lastUpdate || 0),
    Number(task?.memoryUpdatedAt || 0),
    Number(task?.continuationCheckedAt || 0),
  ...
  return {
    ...
    linkedTaskUpdatedAt: Number(task?.lastUpdate || task?.memoryUpdatedAt || task?.createdAt || 0) || null,
    agentId,
    agentName: agent?.name || agentId,
    agentEmoji: agent?.emoji || '🤖',
    agentColor: agent?.color || '#888',
    ...
    state: request.state,
    workflowPhase,
    humanGateReason: task?.humanGateReason || null,
    consensusStatus: task?.consensus?.status || null,
    ...
    updatedAt,
    unresolved: request.state !== 'completed' && workflowPhase !== 'superseded',
  }
}
...
function buildCronAttentionItems(agentMap) {
  ...
  const items = []
  const bossInboxCfg = getBossInboxConfig()
  const autoDisableThreshold = Number(bossInboxCfg.cronAutoDisableAfterErrors || 0)
  const jobsPath = join(getConfig().openclaw?.home || '', 'cron', 'jobs.json')
  for (const job of cronJobs) {
    if (!job?.enabled) continue
    const lastStatus = job?.state?.lastStatus || job?.state?.lastRunStatus || null
    const delivered = job?.state?.lastDelivered === true || String(job?.state?.lastDeliveryStatus || '').trim().toLowerCase() === 'delivered'
    const lastError = String(job?.state?.lastError || '')
    const recoverableStatusNoise = lastStatus === 'error' && delivered && /Edit:\s*`?in ~\/\.openclaw\/agent\/status\.json/i.test(lastError)
    if (recoverableStatusNoise) continue
    ...
    if (autoDisableThreshold > 0 && consecutiveErrors >= autoDisableThreshold && jobsPath && existsSync(jobsPath)) {
      try {
        const jobsData = JSON.parse(readFileSync(jobsPath, 'utf8'))
        const jobsArr = Array.isArray(jobsData?.jobs) ? jobsData.jobs : Array.isArray(jobsData) ? jobsData : []
        const targetJob = jobsArr.find((entry) => entry?.id === job.id)
        if (targetJob && targetJob.enabled) {
          targetJob.enabled = false
          targetJob.autoDisabledAt = Date.now()
          targetJob.autoDisabledReason = `連續 ${consecutiveErrors} 次失敗自動停用`
          targetJob.updatedAtMs = Date.now()
          writeFileSync(jobsPath, `${JSON.stringify(jobsData, null, 2)}\n`)
          console.info(`[boss-inbox] Auto-disabled cron job ${job.id} after ${consecutiveErrors} consecutive errors`)
        }
      } catch (error) {
        console.error('[boss-inbox] Failed to auto-disable cron job:', error.message)
      }
      continue
    }
...
function buildLobsterBrainSummary(tasks, agentMap, requestsById = new Map(), now = Date.now()) {
  const isActiveTask = (task) => !['completed', 'failed'].includes(String(task?.status || '').toLowerCase())
  const isPrimaryTask = (task) => !task?.taskType || task.taskType === 'primary'
  const activeTasks = tasks.filter((task) => isActiveTask(task))
  const childTasksByParent = new Map()

  for (const task of tasks) {
    if (!task?.parentTaskId) continue
    const existing = childTasksByParent.get(task.parentTaskId) || []
    existing.push(task)
    childTasksByParent.set(task.parentTaskId, existing)
  }

  const activeTracks = activeTasks
    .filter((task) => isPrimaryTask(task))
    .map((task) => {
      const brain = task.brainState || {}
      const delegation = task.delegationPlan || {}
      const request = task.requestId ? (requestsById.get(task.requestId) || null) : null
      const origin = classifyLobsterTrackOrigin(task, request)
      const agent = getDisplayAgentMeta(agentMap, task.assignedAgent)
      const rawChildTasks = childTasksByParent.get(task.id) || []
      const childTasks = rawChildTasks
        .map((childTask) => {
          const childBrain = childTask.brainState || {}
          const childAgent = getDisplayAgentMeta(agentMap, childTask.assignedAgent)
          const childConsensus = childTask.consensus || null
          const childFreshestAt = Math.max(
            Number(childTask.memoryUpdatedAt || 0),
            Number(childTask.lastUpdate || 0),
            Number(childTask.startedAt || 0),
            Number(childTask.createdAt || 0),
          )
          return {
            id: childTask.id,
            parentTaskId: childTask.parentTaskId || null,
            rootTaskId: childTask.rootTaskId || childTask.id,
            taskType: childTask.taskType || 'primary',
            mergePolicy: childTask.mergePolicy || null,
            sourceAgent: childTask.sourceAgent || null,
            graphDepth: Number(childTask.graphDepth || 0) || 0,
            title: trimText(childTask.title || childTask.detail || childTask.id || '未命名子任務', 80),
            status: childTask.status || 'pending',
            agentId: childTask.assignedAgent || null,
            agentName: childAgent?.name || childTask.assignedAgent || '未指派',
            agentEmoji: childAgent?.emoji || '🤖',
            summary: trimText(childBrain.summary || '', 100) || null,
            focus: trimText(childBrain.focus || '', 80) || null,
            rootCause: trimText(childTask.rootCause || '', 100) || null,
            blockers: (childBrain.blockers || []).slice(0, 2),
            openLoops: (childBrain.openLoops || []).slice(0, 2),
            consensus: childConsensus
              ? {
                  status: childConsensus.status || 'pending_review',
                  summary: trimText(childConsensus.summary || '', 120) || null,
                  blockingCount: Number(childConsensus.blockingCount || 0),
                  warningCount: Number(childConsensus.warningCount || 0),
                }
              : null,
            riskTier: childTask.riskTier || null,
            retryBudget: Number(childTask.retryBudget || 0),
            retryCount: Number(childTask.retryCount || 0),
            escalationLevel: Number(childTask.escalationLevel || 0),
            humanGateReason: trimText(childTask.humanGateReason || '', 120) || null,
            resolutionSource: childTask.resolutionSource || null,
            closedByParent: Boolean(childTask.closedByParent),
            updatedAt: childFreshestAt || null,
            staleMemory: childFreshestAt > 0 ? (now - childFreshestAt > LOBSTER_MEMORY_STALE_THRESHOLD_MS) : false,
            isActive: isActiveTask(childTask),
          }
        })
        .sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0))
        .slice(0, 6)
      const sidecarDispatches = (delegation.sidecarDispatches || [])
        .slice(0, 4)
        .map((dispatch) => ({
          agentId: dispatch.agentId || null,
          taskId: dispatch.taskId || null,
          agentName: getDisplayAgentMeta(agentMap, dispatch.agentId)?.name || dispatch.agentId || '未知 reviewer',
          status: dispatch.status || null,
          purpose: dispatch.purpose || null,
          dispatchedAt: dispatch.dispatchedAt || null,
          updatedAt: dispatch.updatedAt || dispatch.dispatchedAt || null,
        }))
      const freshestAt = Math.max(
        ...
        Number(task.startedAt || 0),
        Number(task.createdAt || 0),
        ...childTasks.map((entry) => Number(entry.updatedAt || 0)),
      )
      const consensus = task.consensus || null
      const reusableMemory = task.reusableMemory || null
      const reusableRule = reusableMemory?.candidateRule || null
      const workflowPhase = inferTaskWorkflowPhase(task)
      const researchOperatorPhase = inferResearchOperatorPhase(task)
      const delegatedAgents = uniqueList([
        ...(delegation.delegatedAgents || []),
        ...rawChildTasks
          .filter((childTask) => String(childTask.taskType || '') === 'worker_subtask')
          .map((childTask) => childTask.assignedAgent),
        ...sidecarDispatches
          .filter((dispatch) => dispatch.purpose === 'research-followup')
          .map((dispatch) => dispatch.agentId),
      ])
      return {
        id: task.id,
        requestId: task.requestId || null,
        requestSource: origin.requestSource,
        requestFrom: origin.requestFrom,
        originGroup: origin.originGroup,
        originLabel: origin.originLabel,
        originDescription: origin.originDescription,
        rootTaskId: task.rootTaskId || task.id,
        taskType: task.taskType || 'primary',
        mergePolicy: task.mergePolicy || null,
        sourceAgent: task.sourceAgent || null,
        graphDepth: Number(task.graphDepth || 0) || 0,
        title: trimText(task.title || task.detail || task.id || '未命名任務', 80),
        detail: trimText(task.detail || '', 140),
        status: task.status || 'pending',
        workflowPhase,
        researchOperatorPhase,
        milestone: trimText(task.milestone || '', 80) || null,
        nextStep: trimText(task.nextStep || '', 100) || null,
        nextAutoStep: trimText(delegation.nextAutoStep || delegation.nextHandoff || task.nextStep || '', 120) || null,
        agentId: task.assignedAgent || null,
        agentName: agent?.name || task.assignedAgent || '未指派',
        agentEmoji: agent?.emoji || '🤖',
        brainMode: task.brainMode || null,
        objective: trimText(brain.objective || task.title || task.detail || '', 120) || null,
        focus: trimText(brain.focus || '', 90) || null,
        summary: trimText(brain.summary || '', 120) || null,
        nextCheckpoint: trimText(brain.nextCheckpoint || '', 100) || null,
        blockers: (brain.blockers || []).slice(0, 3),
        openLoops: (brain.openLoops || []).slice(0, 3),
        openLoopCount: Array.isArray(brain.openLoops) ? brain.openLoops.length : 0,
        evidence: (brain.evidence || []).slice(0, 2),
        consensus: consensus
          ? {
              status: consensus.status || 'pending_review',
              summary: trimText(consensus.summary || '', 120) || null,
              blockingCount: Number(consensus.blockingCount || 0),
              warningCount: Number(consensus.warningCount || 0),
              resultCount: Number(consensus.resultCount || 0),
              humanApprovalRequired: Boolean(consensus.humanApprovalRequired),
              requiresThirdReviewer: Boolean(consensus.requiresThirdReviewer),
              recommendedAction: trimText(consensus.recommendedAction || '', 80) || null,
            }
          : null,
        reviewerResultCount: Array.isArray(task.reviewerResults) ? task.reviewerResults.length : 0,
        riskTier: task.riskTier || null,
        retryBudget: Number(task.retryBudget || 0),
        retryCount: Number(task.retryCount || 0),
        escalationLevel: Number(task.escalationLevel || 0),
        autoContinueAllowed: Boolean(task.autoContinueAllowed),
        autoApplyAllowed: Boolean(task.autoApplyAllowed),
        humanGateReason: trimText(task.humanGateReason || '', 120) || null,
        rootCause: trimText(task.rootCause || '', 120) || null,
        delegationStatus: delegation.currentStatus || null,
        reviewerMode: delegation.reviewerMode || null,
        suggestedSubagents: (delegation.suggestedSubagents || []).slice(0, 4),
        delegatedAgents,
        sidecarDispatches,
        childTasks,
        childTaskCount: rawChildTasks.length,
        activeChildTaskCount: rawChildTasks.filter((entry) => isActiveTask(entry)).length,
        nextHandoff: trimText(delegation.nextHandoff || '', 100) || null,
        evolutionNote: trimText(task.evolutionNote || '', 120) || null,
        reusableMemory,
        reusableRule: reusableRule
          ? {
              ...
              title: trimText(reusableRule.title || '', 90) || null,
              summary: trimText(reusableRule.summary || '', 120) || null,
              confidence: Number(reusableRule.confidence || 0),
            }
          : null,
        continuationRequired: Boolean(task.continuationRequired),
        pendingAction: task.pendingAction || null,
        completionGateRequired: Boolean(task.completionGateRequired),
        dispatchSessionKey: task.dispatchSessionKey || delegation.sessionKey || null,
        dispatchRunId: task.dispatchRunId || delegation.runId || null,
        memoryUpdatedAt: task.memoryUpdatedAt || null,
        updatedAt: freshestAt || null,
        staleMemory: freshestAt > 0 ? (now - freshestAt > LOBSTER_MEMORY_STALE_THRESHOLD_MS) : false,
      }
    })
    .sort((a, b) => {
      const aHumanGate = a.humanGateReason ? 1 : 0
      const bHumanGate = b.humanGateReason ? 1 : 0
      if (aHumanGate !== bHumanGate) return bHumanGate - aHumanGate
      const aContinuation = a.continuationRequired ? 1 : 0
      const bContinuation = b.continuationRequired ? 1 : 0
      if (aContinuation !== bContinuation) return bContinuation - aContinuation
      const aBlocked = (
        ['blocked', 'failed', 'dispatch_failed'].includes(String(a.delegationStatus || '').toLowerCase())
        || a.blockers.length > 0
        || ['blocked', 'conflict', 'human_gate', 'needs_more_review'].includes(String(a.consensus?.status || '').toLowerCase())
      ) ? 1 : 0
      const bBlocked = (
        ['blocked', 'failed', 'dispatch_failed'].includes(String(b.delegationStatus || '').toLowerCase())
        || b.blockers.length > 0
        || ['blocked', 'conflict', 'human_gate', 'needs_more_review'].includes(String(b.consensus?.status || '').toLowerCase())
      ) ? 1 : 0
      if (aBlocked !== bBlocked) return bBlocked - aBlocked
      return (b.updatedAt || 0) - (a.updatedAt || 0)
    })

  const directIngressTracks = activeTracks.filter((task) => task.originGroup === 'direct_ingress')
  const manualRequestTracks = activeTracks.filter((task) => task.originGroup === 'manual_request')
  const backgroundTracks = activeTracks.filter((task) => task.originGroup === 'background')

  return {
    activeTaskCount: activeTracks.length,
    directIngressTaskCount: directIngressTracks.length,
    manualRequestTaskCount: manualRequestTracks.length,
    backgroundTaskCount: backgroundTracks.length,
    activeChildTaskCount: activeTasks.filter((task) => !isPrimaryTask(task)).length,
    taskGraphNodeCount: activeTracks.reduce((sum, task) => sum + 1 + (task.childTaskCount || 0), 0),
    continuationQueueCount: activeTasks.filter((task) => task.continuationRequired || task.pendingAction === 'continue_after_reply').length,
    delegatedRunCount: activeTasks.filter((task) => task.dispatchSessionKey || task.dispatchRunId || task.delegationPlan?.sessionKey || task.delegationPlan?.runId).length,
    sidecarDispatchCount: activeTracks.reduce((sum, task) => sum + (task.sidecarDispatches?.length || 0), 0),
    rootedCount: tasks.filter((task) => task.rootCause).length,
    evolvingCount: tasks.filter((task) => task.evolutionNote).length,
    staleMemoryCount: activeTracks.filter((task) => task.staleMemory).length,
    humanGateCount: activeTracks.filter((task) => task.humanGateReason).length,
    blockedConsensusCount: activeTracks.filter((task) => ['blocked', 'conflict', 'human_gate'].includes(String(task.consensus?.status || '').toLowerCase())).length,
    reusableRuleCount: activeTracks.filter((task) => Boolean(task.reusableRule)).length,
    blockedTrackCount: activeTracks.filter((task) => task.blockers.length > 0 || ['blocked', 'dispatch_failed'].includes(String(task.delegationStatus || '').toLowerCase())).length,
    directIngressTracks: directIngressTracks.slice(0, 8),
    manualRequestTracks: manualRequestTracks.slice(0, 8),
    backgroundTracks: backgroundTracks.slice(0, 8),
    trackedTasks: activeTracks.slice(0, 8),
  }
}
...
export function buildBossInboxPayload({ skipDigest = false } = {}) {
  ...
  const canonicalPrimaryTasks = requests
    .map((request) => getTaskByRequestId(request.id))
    .filter(Boolean)
  const tasksByRequest = new Map(canonicalPrimaryTasks.map((task) => [task.requestId, task]))
  const tasksById = new Map([...tasks, ...canonicalPrimaryTasks].map((task) => [task.id, task]))
  ...
  const requestItems = requests.map((request) =>
    buildRequestAttentionItem(request, tasksByRequest.get(request.id) || null, eventsByRequest, agentMap)
  )
  ...
  const maintenance = buildMaintenanceAttentionItems(agentMap)
  const attentionItems = syncAttentionItemsWithState(
    [...requestItems, ...cronItems, ...staleItems, ...evolutionItems, ...maintenance.items],
    tasksById,
    requestsById,
    agentMap,
  ).sort((a, b) => {
  ...
  const governanceSummary = {
    ...
    rootMaintenanceStatus: maintenance.snapshot.status,
    rootHygieneIssueCount: maintenance.snapshot.hygieneIssueCount,
    rootCleanupCandidateCount: maintenance.snapshot.cleanupCandidateCount,
    rootCleanupCandidateBytes: maintenance.snapshot.cleanupCandidateBytes,
    copilotRouteStatus: maintenance.snapshot.routeSummary?.status || 'unknown',
    copilotRouteDegraded: Boolean(maintenance.snapshot.routeDegraded),
    copilotRouteStale: Boolean(maintenance.snapshot.routeStale),
  }
  ...
  const reusableRules = listLobsterRules(12)
  const lobsterBrain = {
    ...buildLobsterBrainSummary(tasks, agentMap, requestsById, now),
    reusableRules,
  }
  ...
  return {
    ...
    lobsterBrain,
    reusableRules,
    rootMaintenance: maintenance.snapshot,
  }
}
...
export function runAttentionAction(id, {
  ...
  if (normalizedAction === 'save_reply') {
    const baseItem = item || {
      id,
      source: existingState?.source || 'attention-state',
      agentId: existingState?.agentId || null,
    ...
    const nextTaskResult = String(taskResult ?? note ?? '').trim() || null
    const savedState = upsertAttentionState({
      ...toAttentionStateRow(baseItem, existingState || null),
      taskResult: nextTaskResult,
      lastFeedbackAt: nextTaskResult ? now : (existingState?.lastFeedbackAt || null),
      actionHistory: appendAttentionActionHistory(existingState || {}, {
        action: 'save_reply',
        actor: reviewer,
        result: nextTaskResult ? trimText(nextTaskResult, 120) : 'cleared',
      }),
      updatedAt: now,
    })
    return {
      attentionItem:
        getAttentionItemById(id) ||
        (item
          ? mergeAttentionItemWithState(item, savedState)
          : buildAttentionItemFromState(savedState)),
      linkedRequest: savedState.linkedRequestId ? getRequestById(savedState.linkedRequestId) : null,
      linkedTask: savedState.linkedTaskId ? getTaskById(savedState.linkedTaskId) : null,
      note: nextTaskResult,
      reviewer,
