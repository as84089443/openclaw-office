...
function getMaintenanceReportsDir() {
  return join(getConfig().openclaw?.home || '', 'artifacts', 'maintenance', 'reports')
}
...
function getRootMaintenanceSnapshot() {
  ...
  const hygiene = parseJsonSnapshot(join(reportsDir, 'latest-root-hygiene.json'))
  const cleanup = parseJsonSnapshot(join(reportsDir, 'latest-root-maintenance.json'))
  ...
  const reportPresent = Boolean(hygiene || cleanup)
  const snapshotAgeMs = Math.max(0, Date.now() - generatedAt)
  const stale = !reportPresent || snapshotAgeMs > MAINTENANCE_STALE_THRESHOLD_MS
  ...
  return {
    ...
    reportPaths: {
      hygiene: hygiene?.reportPath || join(reportsDir, 'latest-root-hygiene.json'),
      cleanup: cleanup?.reportPath || join(reportsDir, 'latest-root-maintenance.json'),
    },
    issueBreakdown: {
      unexpectedRootFiles,
...
function buildMaintenanceAttentionItems(agentMap) {
  ...
  if (snapshot.stale) {
    detailParts.push(`maintenance reports stale ${Math.round(snapshot.snapshotAgeMs / (60 * 60 * 1000))}h`)
  }
  if (hasHygieneIssues) {
    detailParts.push(
  ...
  detailParts.push(`reports: ${snapshot.reportPaths.hygiene} / ${snapshot.reportPaths.cleanup}`)
    ...
    items: [
      {
        ...
        source: 'maintenance',
        latestEventId: null,
        signalCount: Math.max(snapshot.hygieneIssueCount + snapshot.cleanupCandidateCount, 1),
        ...
        attentionType,
        priority,
        severity: priority + Math.min(snapshot.hygieneIssueCount, 5) + Math.min(snapshot.cleanupCandidateCount, 8),
        commercialValue: 0,
        needsDecision: false,
        state: 'open',
        title: hasHygieneIssues
        ...
        escalationReason: snapshot.stale ? 'maintenance-stale' : (hasHygieneIssues ? 'root-hygiene' : 'cleanup-pending'),
      },
    ],
  }
}
