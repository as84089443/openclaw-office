...
import { listAgentSessionDirectories } from './session-routing.js'
...
function readSessionFilesDirectly() {
  ...
  if (sessionDirs.length === 0) {
    console.error('OpenClaw sessions directories not found')
    return null
  }

  const sessions = []
  
  // Get all session files with their modification times across all fish.
  const allFiles = sessionDirs
    .flatMap(({ dir }) => (
      existsSync(dir)
        ? readdirSync(dir)
