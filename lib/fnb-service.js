...
import {
  ...
  shouldUseExistingDraftFallbackForInstruction,
...
function buildApprovalMessage(draft, locationId) {
  if (draft.payload?.origin === 'merchant-copilot') {
    const preview = String(draft.body || '').replace(/\s+/g, ' ').trim()
    return [
      '我先幫你整理好一版草稿。',
      `重點：${draft.title}`,
      preview ? `預覽：${preview.length > 72 ? `${preview.slice(0, 71)}…` : preview}` : null,
...
async function buildMerchantCopilotTaskContextInternal(context, locationId, instruction, taskType, sourceDraftId = null) {
  ...
  if (!sourceDraft && taskType === 'rewrite-copy' && shouldUseExistingDraftFallbackForInstruction(instruction)) {
...
async function submitMerchantCopilotMessageInternal(context, { lineUserId, locationId = null, text, externalEventId = null, source = 'line' }) {
  ...
  if (persisted.inserted && shouldSynchronouslyCompleteMerchantCopilotTask()) {
    const completed = await completeMerchantCopilotTaskInternal(context, task.id)
    return {
      ok: Boolean(completed?.ok),
      status: completed?.draft ? 'draft-ready' : (completed?.task?.status || 'completed'),
      replyText: completed?.draft
        ? buildMerchantCopilotDraftReadyReply(completed.draft)
        : (completed?.ok
            ? '我先幫你整理好一版草稿，請直接看待審核項目。'
            : '這則需求我先交給營運協助處理，稍後再回你。'),
      threadId: thread.id,
      task: completed?.task || task,
      draft: completed?.draft || null,
    }
  }
...
function buildLineReplyText(result) {
  ...
  if (result.status === 'binding-required') return '先完成商家身份綁定後，我才能繼續幫你處理。'
  if (result.status === 'onboarding-required') return '目前還沒有可綁定的店家據點，請先完成 onboarding。'
...
function shouldSynchronouslyCompleteMerchantCopilotTask() {
  return process.env.FNB_MERCHANT_COPILOT_SYNC_FALLBACK !== '0'
}

function buildMerchantCopilotDraftReadyReply(draft) {
  if (!draft) return '我先幫你整理好一版草稿，已送出待審核卡片。'
  const preview = String(draft.body || '').replace(/\s+/g, ' ').trim()
  return [
    '我先幫你整理好一版草稿，已送出待審核卡片。',
    `重點：${draft.title || '商家文案草稿'}`,
    preview ? `預覽：${preview.length > 48 ? `${preview.slice(0, 47)}…` : preview}` : null,
    '直接到待審核按同意 / 再改 / 跳過就可以。',
  ].filter(Boolean).join('\n')
}

async function buildMerchantLineIdentityReplyInternal(context) {
  const defaultLocationId = await resolveLocationId(context, null)
  if (defaultLocationId) {
    return {
      ok: false,
      status: 'binding-required',
      replyText: [
        '先完成商家身份綁定後，我才能幫你處理待審核、文案與設定。',
        `點這裡完成綁定：${buildMerchantBindUrl(defaultLocationId)}`,
      ].join('\n'),
    }
  }
  ...
  return {
    ...
    replyText: [
      '目前還沒有可綁定的店家據點，請先完成 onboarding。',
      `營運入口：${buildOpsDashboardUrl(null)}`,
    ].join('\n'),
  }
}
...
export async function processLineWebhook(rawBody, signature) {
  ...
  for (const event of payload.events || []) {
    ...
    if ((command || (event.type === 'message' && event.message?.type === 'text')) && !locationId) {
      const result = await buildMerchantLineIdentityReplyInternal(context)
      if (event.replyToken) {
        await context.lineAdapter.replyText(event.replyToken, buildLineReplyText(result))
      }
      processed.push({ externalEventId, status: 'binding-required', result })
      continue
    }
