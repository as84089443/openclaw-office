...
function getPublicBaseUrl(fallback = 'http://localhost:4200') {
  return process.env.FNB_PUBLIC_BASE_URL || fallback
}
...
function buildOnboardingChecklist({ merchantProfile, brandPack, menuItems, channels }) {
  ...
  return [
    ...
    },
    {
      id: 'google-channel',
      label: 'Google 商家串接',
      done: googleChannel?.status === 'connected',
    },
  ]
}

function buildAlerts({ channels, pendingApprovals, pipeline }) {
  const alerts = []
...
async function getOpsSnapshotInternal(context, locationId) {
  ...
  const channels = (await context.channelRepository.listConnections(resolvedLocationId)).map((row) => normalizeConnection(row))
  ...
  const onboardingChecklist = buildOnboardingChecklist({ merchantProfile, brandPack, menuItems, channels })
  ...
  const alerts = buildAlerts({ channels, pendingApprovals, pipeline })
...
export async function createLineAuthUrl({ locationId, origin, redirectTo = '/merchant' }) {
  const context = await getContext()
  const resolvedLocationId = await resolveLocationId(context, locationId)
  const redirectUri = `${getPublicBaseUrl(origin)}/api/auth/line/callback`
...
export async function createGoogleAuthUrl({ locationId, origin, redirectTo = '/ops', googleLocationName = null }) {
  const context = await getContext()
  const resolvedLocationId = await resolveLocationId(context, locationId)
  const redirectUri = `${getPublicBaseUrl(origin)}/api/auth/google/callback`
