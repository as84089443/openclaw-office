...
function buildDiagnostics({
  ...
  if (!openclawConfig?.parsed) {
    warnings.push({
      ...
      severity: 'error',
      message: '找不到 canonical openclaw.json，AI Team 與 BW Office 只能顯示降級資料。',
    })
  } else if (!agentCount) {
    warnings.push({
  ...
  if (
    ...
    warnings.push({
      ...
      severity: 'warning',
      message: '部署站的 gateway 目前未連到真實 OpenClaw runtime，legacy realtime 面板會是降級狀態。',
    })
  }
...
function normalizeDiscordChannelPolicy(policy = {}) {
  const mode = String(policy?.mode || '').trim() || null
  const defaultDelivery = String(policy?.defaultDelivery || '').trim() || null
  const chatEscapePrefix = String(policy?.chatEscapePrefix || '').trim() || null
  const operatorMode = mode === 'autonomous_operator' ? 'strict' : null
  return {
    allow: policy?.allow !== undefined ? Boolean(policy.allow) : true,
    requireMention: policy?.requireMention === undefined ? null : Boolean(policy.requireMention),
    mode,
    operatorMode,
    defaultDelivery,
    chatEscapePrefix,
    workflowOnly: defaultDelivery === 'workflow_only',
    autonomousByDefault: defaultDelivery === 'workflow_only' || mode === 'autonomous_operator',
  }
}

function extractDiscordGuildPolicies(config = {}) {
  return (
    config?.channelPolicies?.discord?.guilds
    || config?.channels?.discord?.guilds
    || null
  )
}

function normalizeDiscordPolicies(...configs) {
  const normalized = {
    byGuild: {},
    byChannel: {},
  }
  ...
  for (const config of configs) {
    const guilds = extractDiscordGuildPolicies(config)
    if (!guilds || typeof guilds !== 'object') continue

    for (const [guildId, guildConfig] of Object.entries(guilds)) {
      const channels = guildConfig?.channels
      if (!channels || typeof channels !== 'object') continue
      normalized.byGuild[guildId] = normalized.byGuild[guildId] || {}
      for (const [channelId, channelConfig] of Object.entries(channels)) {
        const policy = normalizeDiscordChannelPolicy(channelConfig)
        normalized.byGuild[guildId][channelId] = policy
        normalized.byChannel[channelId] = policy
      }
    }
  }
...
function buildConfig() {
  ...
  config.channelPolicies = {
    discord: normalizeDiscordPolicies(openclawConfig.parsed, fileConfig || {}),
  }
  config.openclaw = {
    home: openclawHome,
  ...
  const openclawGateway = openclawConfig?.parsed?.gateway || {}
  const openclawGatewayAuth = openclawGateway?.auth || {}
  const openclawTelegram = openclawConfig?.parsed?.channels?.telegram || {}
  if (openclawGateway?.url) config.gateway.url = openclawGateway.url
  else if (openclawGateway?.port) config.gateway.url = `ws://127.0.0.1:${openclawGateway.port}`
  if (openclawGatewayAuth?.token) config.gateway.token = openclawGatewayAuth.token
  if (openclawTelegram?.botToken) config.telegram.botToken = openclawTelegram.botToken
  if (openclawTelegram?.chatId) config.telegram.chatId = openclawTelegram.chatId
  if (openclawTelegram?.webhookSecret) config.telegram.webhookSecret = openclawTelegram.webhookSecret
  if (openclawTelegram?.webhookUrl) config.telegram.webhookUrl = openclawTelegram.webhookUrl
  if (openclawTelegram?.webhookPath) config.telegram.webhookPath = openclawTelegram.webhookPath
  if (openclawTelegram?.webhookHost) config.telegram.webhookHost = openclawTelegram.webhookHost
  if (openclawTelegram?.webhookPort) {
    config.telegram.webhookPort = normalizePort(
      openclawTelegram.webhookPort,
      config.telegram.webhookPort,
    )
  }
  ...
  if (process.env.TELEGRAM_WEBHOOK_URL) config.telegram.webhookUrl = process.env.TELEGRAM_WEBHOOK_URL
  if (process.env.TELEGRAM_WEBHOOK_PATH) config.telegram.webhookPath = process.env.TELEGRAM_WEBHOOK_PATH
  if (process.env.TELEGRAM_WEBHOOK_HOST) config.telegram.webhookHost = process.env.TELEGRAM_WEBHOOK_HOST
...
export function getDiscordChannelPolicy({ guildId = null, channelId = null } = {}) {
  const normalizedChannelId = String(channelId || '').trim()
  if (!normalizedChannelId) return null

  const policies = getConfig().channelPolicies?.discord || {}
  const normalizedGuildId = String(guildId || '').trim()

  if (normalizedGuildId) {
    const guildPolicy = policies.byGuild?.[normalizedGuildId]?.[normalizedChannelId]
    if (guildPolicy) return { ...guildPolicy, guildId: normalizedGuildId, channelId: normalizedChannelId }
  }

  const fallback = policies.byChannel?.[normalizedChannelId]
  return fallback ? { ...fallback, guildId: normalizedGuildId || null, channelId: normalizedChannelId } : null
}
