...
const FNB_SCHEMA_SQL = `
  ...
  CREATE TABLE IF NOT EXISTS fnb_merchant_messages (
    ...
    FOREIGN KEY (thread_id) REFERENCES fnb_merchant_threads(id),
    FOREIGN KEY (tenant_id) REFERENCES fnb_tenants(id),
    FOREIGN KEY (location_id) REFERENCES fnb_locations(id),
    FOREIGN KEY (operator_id) REFERENCES fnb_operator_accounts(id)
  );
  ...
  CREATE TABLE IF NOT EXISTS fnb_merchant_tasks (
    ...
    FOREIGN KEY (location_id) REFERENCES fnb_locations(id),
    FOREIGN KEY (operator_id) REFERENCES fnb_operator_accounts(id),
    FOREIGN KEY (output_draft_id) REFERENCES fnb_drafts(id)
  );
  ...
  CREATE TABLE IF NOT EXISTS fnb_merchant_task_events (
    ...
    payload_json TEXT,
    created_at BIGINT NOT NULL,
    FOREIGN KEY (task_id) REFERENCES fnb_merchant_tasks(id),
    FOREIGN KEY (location_id) REFERENCES fnb_locations(id)
  );
  ...
  CREATE TABLE IF NOT EXISTS fnb_merchant_prompt_profiles (
    ...
    preferred_ctas_json TEXT,
    promo_preferences_json TEXT,
    updated_at BIGINT NOT NULL,
    FOREIGN KEY (location_id) REFERENCES fnb_locations(id)
  );
...
export class FnbChannelConnectionRepository {
  ...
  assertLocationId(locationId) {
    ...
    const error = new Error('No merchant location is available for channel connection. Complete onboarding first.')
  ...
  async saveConnection(connection) {
    this.assertLocationId(connection?.locationId)
...
export class FnbCampaignRepository {
  ...
  async listPendingApprovals(locationId) {
    return this.adapter.all(`
      SELECT a.*, d.title, d.body, d.risk_score, d.route, d.status AS draft_status, d.scheduled_for, d.draft_type, d.payload_json
...
export class FnbMerchantCopilotRepository {
  constructor(adapter) {
    this.adapter = adapter
  }

  async getPromptProfile(locationId) {
    return this.adapter.get('SELECT * FROM fnb_merchant_prompt_profiles WHERE location_id = ?', [locationId])
  }

  async savePromptProfile(profile) {
    const existing = await this.getPromptProfile(profile.locationId)
    if (existing) {
      await this.adapter.run(`
        UPDATE fnb_merchant_prompt_profiles
        SET preferred_language = ?, tone_summary = ?, forbidden_phrases_json = ?, preferred_ctas_json = ?, promo_preferences_json = ?, updated_at = ?
        WHERE location_id = ?
      `, [
        profile.preferredLanguage || 'zh-TW',
        profile.toneSummary || null,
        toJson(profile.forbiddenPhrases || []),
        toJson(profile.preferredCtas || []),
        toJson(profile.promoPreferences || {}),
        profile.updatedAt,
        profile.locationId,
      ])
      return
    ...
    await this.adapter.run(`
      INSERT INTO fnb_merchant_prompt_profiles (location_id, preferred_language, tone_summary, forbidden_phrases_json, preferred_ctas_json, promo_preferences_json, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `, [
      profile.locationId,
      profile.preferredLanguage || 'zh-TW',
  ...
  async findThreadByOperator(locationId, operatorId, source = 'line') {
    return this.adapter.get(`
      SELECT *
      FROM fnb_merchant_threads
      WHERE location_id = ? AND operator_id = ? AND source = ?
      LIMIT 1
    `, [locationId, operatorId, source])
  }

  async getThread(threadId) {
    return this.adapter.get('SELECT * FROM fnb_merchant_threads WHERE id = ?', [threadId])
  }

  async saveThread(thread) {
    const existing = thread.id
      ? await this.getThread(thread.id)
      : await this.findThreadByOperator(thread.locationId, thread.operatorId, thread.source || 'line')

    if (existing) {
      await this.adapter.run(`
        UPDATE fnb_merchant_threads
        SET status = ?, title = ?, summary = ?, latest_task_id = ?, metadata_json = ?, last_message_at = ?, updated_at = ?
        WHERE id = ?
      `, [
        thread.status ?? existing.status,
        thread.title ?? existing.title,
        thread.summary ?? existing.summary,
        thread.latestTaskId ?? existing.latest_task_id ?? null,
        toJson(thread.metadata ?? fromJson(existing.metadata_json, {})),
        thread.lastMessageAt ?? existing.last_message_at ?? null,
        thread.updatedAt ?? existing.updated_at,
        existing.id,
      ])
      return existing.id
    }
    ...
    await this.adapter.run(`
      INSERT INTO fnb_merchant_threads (id, tenant_id, location_id, operator_id, source, status, title, summary, latest_task_id, metadata_json, last_message_at, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      id,
      thread.tenantId,
  ...
  async listThreads(locationId, { operatorId = null, limit = 8 } = {}) {
  ...
  async findMessageByExternalEvent(source, externalEventId) {
    if (!externalEventId) return null
    return this.adapter.get(`
      SELECT *
      FROM fnb_merchant_messages
  ...
  async getMessage(messageId) {
    return this.adapter.get('SELECT * FROM fnb_merchant_messages WHERE id = ?', [messageId])
  }

  async createMessage(message) {
    const existing = await this.findMessageByExternalEvent(message.source, message.externalEventId)
    if (existing) {
      return { inserted: false, id: existing.id }
    }
    ...
    try {
      await this.adapter.run(`
        INSERT INTO fnb_merchant_messages (id, thread_id, tenant_id, location_id, operator_id, external_event_id, role, source, message_type, intent, body, metadata_json, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        id,
        message.threadId,
      ...
      const duplicated = await this.findMessageByExternalEvent(message.source, message.externalEventId)
      if (duplicated) {
        return { inserted: false, id: duplicated.id }
      }
  ...
  async findTaskByDedupeKey(locationId, dedupeKey) {
    if (!dedupeKey) return null
    return this.adapter.get(`
      SELECT *
      FROM fnb_merchant_tasks
      WHERE location_id = ? AND dedupe_key = ?
      LIMIT 1
    `, [locationId, dedupeKey])
  }

  async getTask(taskId) {
    return this.adapter.get('SELECT * FROM fnb_merchant_tasks WHERE id = ?', [taskId])
  }

  async createTask(task) {
    const existing = await this.findTaskByDedupeKey(task.locationId, task.dedupeKey)
    if (existing) {
      return { inserted: false, id: existing.id }
    }
    ...
    try {
      await this.adapter.run(`
        INSERT INTO fnb_merchant_tasks (id, thread_id, tenant_id, location_id, operator_id, task_type, status, source, dedupe_key, title, instruction_text, context_json, output_draft_id, confidence, assigned_to, handoff_ref, error_message, started_at, completed_at, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        id,
        task.threadId,
      ...
      const duplicated = await this.findTaskByDedupeKey(task.locationId, task.dedupeKey)
      if (duplicated) {
        return { inserted: false, id: duplicated.id }
      }
      throw new Error('Failed to create merchant task')
    }
  }

  async updateTask(taskId, patch = {}, expectedStatuses = []) {
    const existing = await this.getTask(taskId)
    if (!existing) return null
    if (expectedStatuses.length > 0 && !expectedStatuses.includes(existing.status)) return null

    await this.adapter.run(`
      UPDATE fnb_merchant_tasks
      SET status = ?, title = ?, context_json = ?, output_draft_id = ?, confidence = ?, assigned_to = ?, handoff_ref = ?, error_message = ?, started_at = ?, completed_at = ?, updated_at = ?
      WHERE id = ?
    `, [
      patch.status ?? existing.status,
      patch.title ?? existing.title,
      toJson(patch.context ?? fromJson(existing.context_json, {})),
      patch.outputDraftId ?? existing.output_draft_id ?? null,
      patch.confidence ?? existing.confidence ?? null,
      patch.assignedTo ?? existing.assigned_to ?? null,
      patch.handoffRef ?? existing.handoff_ref ?? null,
  ...
  async listTasks(locationId, { threadId = null, statuses = [], limit = 8 } = {}) {
    const params = [locationId]
    let sql = `
      SELECT *
      FROM fnb_merchant_tasks
  ...
  async claimNextTask(assignedTo = 'openclaw') {
    const candidate = await this.adapter.get(`
      SELECT *
      FROM fnb_merchant_tasks
    ...
    return this.updateTask(candidate.id, {
      status: 'in_progress',
      assignedTo,
      startedAt: Date.now(),
      updatedAt: Date.now(),
  ...
  async createTaskEvent(event) {
    await this.adapter.run(`
      INSERT INTO fnb_merchant_task_events (id, task_id, location_id, event_type, payload_json, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
    `, [
      event.id || randomUUID(),
      event.taskId,
  ...
  async listTaskEvents(taskId, limit = 20) {
    return this.adapter.all(`
      SELECT *
      FROM fnb_merchant_task_events
      WHERE task_id = ?
      ORDER BY created_at DESC
      LIMIT ?
    `, [taskId, limit])
