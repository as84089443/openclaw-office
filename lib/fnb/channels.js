  ...
  return {
    ...
    async pushApprovalRequest(to, approval) {
      ...
      const scheduledText = approval.scheduledFor
        ...
        : '待排程'
      const middleAction = approval.rewriteEnabled
        ? { type: 'postback', label: '再改', data: `fnb:rewrite:${approval.draftId}` }
...
export function parseLineMerchantCommand(input) {
  ...
  const match = text.match(/^(同意|延後|再改|跳過|approve|reschedule|rewrite|skip)\s+([A-Za-z0-9_-]+)/i)
  ...
  if (action === '再改' || action === 'rewrite') return { messageIntent: 'rewrite-draft', payload: { draftId } }
