...
function resolvePersistedRequestId(requestId) {
  if (!requestId) return null
  return getRequestByIdStmt.get(requestId)?.id || null
}
...
export function createTask(data) {
  ...
  const row = {
    id,
    request_id: resolvePersistedRequestId(data.requestId),
...
export function updateTask(id, data) {
  ...
  const row = {
    id,
    request_id: hasField('requestId') ? resolvePersistedRequestId(data.requestId) : (existing.request_id ?? null),
