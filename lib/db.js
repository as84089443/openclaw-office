...
export const DB_PATH = process.env.OPENCLAW_OFFICE_DB_PATH || join(process.cwd(), 'data', 'openclaw-office.db')
...
const insertTaskStmt = db.prepare(`
  INSERT INTO tasks (id, request_id, title, detail, assigned_agent, status, attention_type, priority, needs_decision, estimated_value, completion_value, did_improve, did_improve_score, business_delta, process_score, business_score, rollback_needed, created_at, started_at, completed_at, result)
  VALUES (@id, @request_id, @title, @detail, @assigned_agent, @status, @attention_type, @priority, @needs_decision, @estimated_value, @completion_value, @did_improve, @did_improve_score, @business_delta, @process_score, @business_score, @rollback_needed, @created_at, @started_at, @completed_at, @result)
`)
