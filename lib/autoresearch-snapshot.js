...
async function readRunArtifact(dir) {
  ...
  const [dirStat, manifest, summary, qa, revalidationReport, revalidation, memoryHandoff, codexLastMessage, eventLines] = await Promise.all([
    stat(dir),
...
export async function getAutoResearchSnapshot() {
  ...
  const [strategyState, history, latestStrategyReportPath, latestPlanFallbackPath, searchSpaceRaw, researchMemoryRaw, proxyHealth, baselineRun, codexRun, overnightLogPath, branch, head] = await Promise.all([
  ...
  const revalidationContent = codexRun?.revalidationReport?.content || null
  ...
  const currentLane = latestPlan?.selectedLaneId
    ? {
  ...
  const nextLane = latestPlan?.nextLaneId
    ? {
  ...
  return {
    ...
    metrics: {
      ...
      revalidationVerdict: toStringSafe(codexRun?.revalidation?.verdict),
      revalidationMeanValBpb: toNumberSafe(codexRun?.revalidation?.meanValBpb),
      revalidationSpread: toNumberSafe(codexRun?.revalidation?.valSpread),
      revalidationMeanMemoryGb: toNumberSafe(codexRun?.revalidation?.meanMemoryGb),
    ...
    artifacts: {
      ...
      revalidationReport: codexRun?.revalidationReport || null,
